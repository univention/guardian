...
package through the regular errata process (see `univention-keycloak` in the
keycloak-app repo for a package released the same way) — there is nothing
Guardian-specific about the release mechanics.
...
every branch. Only when bumping a version do you need to clickk the manual `mirror-cerbos-image`
