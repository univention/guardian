...
class PermissionStaticDataAdapter(PermissionPersistencePort):
    class Config:
        alias = "in_memory"
    ...
    async def update(self, permission: Permission) -> Permission:
        p: Permission
        for idx, p in enumerate(self._data["permissions"]):
            if (
                p.name == permission.name
                and p.app_name == permission.app_name
                and p.namespace_name == permission.namespace_name
            ):
                self._data["permissions"][idx] = permission
                return permission
        raise ObjectNotFoundError(f"Object {permission.name} not found.")
        ...
        try:
            return next(
                (
                    ...
                    if permission.name == query.name
                )
            )
        except StopIteration:
            raise ObjectNotFoundError(f"Object {query.name} not found.")

    async def read_many(
        self,
        query: PermissionsGetQuery,
    ) -> PersistenceGetManyResult[Permission]:
        permission_data: List[Permission] = self._data["permissions"]

        if query.app_name is not None:
            permission_data = [
                d for d in permission_data if d.app_name == query.app_name
            ]

        if query.namespace_name is not None:
            permission_data = [
                d for d in permission_data if d.namespace_name == query.namespace_name
            ]
        ...
        if query.pagination.query_limit:
            ...
            result = permission_data[query.pagination.query_offset :]
