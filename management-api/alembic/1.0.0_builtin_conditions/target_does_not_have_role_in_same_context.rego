...
# Helper to check if role/context pair does NOT match
_role_context_mismatch(actor_context, parameters, target_role_and_context) if {
...
condition("guardian:builtin:target_does_not_have_role_in_same_context", parameters, condition_data) if {
	...
	every target_crole in condition_data.target.old.roles {
		...
		_role_context_mismatch(actor_context, parameters, target_role_and_context)
	}
} else := false
