...
test('navigation', async ({page}) => {
  ...
  await expect(page).toHaveURL(/\/guardian\/management-ui\/namespaces/);
  await page.getByRole('link', {name: 'Contexts', exact: true}).click();
  await expect(page).toHaveURL(/\/guardian\/management-ui\/contexts/);
  ...
  await expect(page).toHaveURL(/\/guardian\/management-ui\/roles\/edit\/app-1:namespace-1:role-1\/capabilties/);
