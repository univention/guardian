...
def mock_get_jwk_set(monkeypatch):
    ...
    monkeypatch.setattr(jwt.jwks_client.PyJWKClient, "get_jwk_set", mock_get_jwk_set)


@pytest_asyncio.fixture
async def auth_adapter_oauth(monkeypatch, mock_get_jwk_set):
    ...
    registry.register_port(AuthenticationPort)
    registry.register_adapter(AuthenticationPort, adapter_cls=FastAPIOAuth2)
    with monkeypatch.context() as m:
        m.setenv("OAUTH_ADAPTER__WELL_KNOWN_URL", "/dev/zero")
        m.setattr(requests, "get", mock_get)
        adapter = await registry.request_adapter(AuthenticationPort, FastAPIOAuth2)
    return adapter
