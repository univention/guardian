...
def test_hr1_add_policy_makes_kind_decidable(cerbos, scratch_dir):
    ...
    assert baseline['r-1']['view'] == 'EFFECT_DENY', (
    ...
    wait_until(
        ...
        message=(f'{SCRATCH_KIND}.view did not flip to ALLOW within {RELOAD_TIMEOUT}s of dropping the policy'),
    )
    ...
    assert sanity['r-1']['view'] == 'EFFECT_ALLOW', 'shipped document policy stopped serving during reload'
