...
POLICIES_DIR = Path(os.environ.get('CERBOS_POLICIES_DIR', '/usr/share/univention-guardian-server/policies'))
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if last:
            return last
        time.sleep(interval)
    raise AssertionError(message or f'predicate did not become true within {timeout}s (last={last!r})')
...
def check_resources(
    ...
    *,
    ...
    resp = session.post(f'{CERBOS_BASE_URL}/api/check/resources', json=payload, timeout=10)
    resp.raise_for_status()
    data = resp.json()
    return {result['resource']['id']: dict(result.get('actions', {})) for result in data.get('results', [])}
